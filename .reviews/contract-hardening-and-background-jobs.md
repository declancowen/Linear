...
**Where:** [lib/server/convex/notifications.ts](/Users/declancowen/Documents/GitHub/Linear/lib/server/convex/notifications.ts:135), [convex/app/email_job_handlers.ts](/Users/declancowen/Documents/GitHub/Linear/convex/app/email_job_handlers.ts:68)
...
**Where:** [scripts/send-email-jobs.mjs](/Users/declancowen/Documents/GitHub/Linear/scripts/send-email-jobs.mjs:27)
...
**Where:** [lib/store/app-store-internal/slices/workspace.ts](/Users/declancowen/Documents/GitHub/Linear/lib/store/app-store-internal/slices/workspace.ts:674)
...
**Where:** [scripts/send-notification-digests.mjs](/Users/declancowen/Documents/GitHub/Linear/scripts/send-notification-digests.mjs:232), [convex/app/notification_handlers.ts](/Users/declancowen/Documents/GitHub/Linear/convex/app/notification_handlers.ts:61)
...
**Where:** [convex/app.ts](/Users/declancowen/Documents/GitHub/Linear/convex/app.ts:357), [convex/app.ts](/Users/declancowen/Documents/GitHub/Linear/convex/app.ts:382)
...
- `sendTeamRemovalEmails` in [lib/server/email.ts](/Users/declancowen/Documents/GitHub/Linear/lib/server/email.ts:675) is still inline Resend delivery, but it is currently dead code in this branch (`rg` finds no call sites). That is cleanup / consistency debt, not a live bug in the PR diff.
- The duplication note against [lib/store/app-store-internal/domain-updates.ts](/Users/declancowen/Documents/GitHub/Linear/lib/store/app-store-internal/domain-updates.ts:73) is fair maintainability feedback, but not a correctness finding for this review.
- The `collect()` note against [convex/app/email_job_handlers.ts](/Users/declancowen/Documents/GitHub/Linear/convex/app/email_job_handlers.ts:108) is a real scale watchpoint, but it is not a blocking bug at current branch scope.
- The fire-and-forget refresh change in [lib/store/app-store-internal/runtime.ts](/Users/declancowen/Documents/GitHub/Linear/lib/store/app-store-internal/runtime.ts:29) is an intentional ordering tradeoff. It weakens the old blocking guarantee, but the code is internally consistent with the branch’s broader shift toward non-blocking reconciliation.
...
- **Resolved:** `F1-01` Retrying `enqueueEmailJobsServer` can duplicate outbound emails because the enqueue mutation is not idempotent.
  Fixed by removing transport-level retries from [lib/server/convex/notifications.ts](/Users/declancowen/Documents/GitHub/Linear/lib/server/convex/notifications.ts).

- **Resolved:** `F1-03` `scripts/send-email-jobs.mjs` exited successfully even when some or all email jobs failed.
  Fixed by returning batch counts and setting `process.exitCode = 1` when any job fails in [scripts/send-email-jobs.mjs](/Users/declancowen/Documents/GitHub/Linear/scripts/send-email-jobs.mjs).

- **Resolved:** `F2-01` `joinTeamByCode` no longer refreshed or patched local state.
  Fixed by restoring immediate reconciliation in [lib/store/app-store-internal/slices/workspace.ts](/Users/declancowen/Documents/GitHub/Linear/lib/store/app-store-internal/slices/workspace.ts).

- **Resolved:** `F2-02` A single digest-send failure could leave later claimed digests locked until TTL expiry.
  Fixed by releasing the current and still-unprocessed claimed digests before aborting in [scripts/send-notification-digests.mjs](/Users/declancowen/Documents/GitHub/Linear/scripts/send-notification-digests.mjs).
...
- **Resolved:** `F2-03` Digest claim/release mutations still bumped `snapshotVersion` even though they only mutate operational claim metadata.
  Fixed by moving the digest claim/mark/release exports onto `operationalMutation(...)` in [convex/app.ts](/Users/declancowen/Documents/GitHub/Linear/convex/app.ts).
...
- **Resolved:** Claiming email jobs could still send per-notification emails after a digest had already marked the underlying notification emailed.
  Fixed by retiring already-covered notification-linked jobs during claim in [convex/app/email_job_handlers.ts](/Users/declancowen/Documents/GitHub/Linear/convex/app/email_job_handlers.ts).

- **Resolved:** A post-send acknowledgement failure in the email-job worker could resend the same email on the next run.
  Fixed by separating send vs. acknowledgement failure handling and passing a stable provider idempotency key (`job.id`) in [scripts/send-email-jobs.mjs](/Users/declancowen/Documents/GitHub/Linear/scripts/send-email-jobs.mjs).
