...
const searchResultLabels: Record<GlobalSearchResult["kind"], string> = {
  document: "Document",
  item: "Work item",
...
function searchResultLabel(kind: GlobalSearchResult["kind"]) {
  return searchResultLabels[kind]
}

function getSearchResultMetadata(
  result: GlobalSearchResult,
  team: WorkspaceSearchTeam | null
) {
  return [
  ...
  return (
    <div className="flex flex-col gap-3 border-b border-border/60 pb-4">
      ...
      <div className="flex flex-wrap items-center gap-2">
        ...
        <Select
          value={status}
          onValueChange={(value) => setStatus(value as SearchStatusFilter)}
        >
          <SelectTrigger className="w-full sm:w-40">
...
function getSearchResultTeam(
  result: GlobalSearchResult,
  teamsById: Map<string, WorkspaceSearchTeam>
) {
  return result.teamId ? (teamsById.get(result.teamId) ?? null) : null
}
        ...
        <div className="space-y-0">
          {filteredResults.map((result) => (
            <WorkspaceSearchResultRow
              key={result.id}
              result={result}
              team={getSearchResultTeam(result, teamsById)}
            />
          ))}
        </div>
      )}
